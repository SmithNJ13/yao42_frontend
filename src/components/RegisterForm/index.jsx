  ...
  return (
    <div id='page' className='tw-flex tw-h-screen tw-justify-center tw-items-center tw-w-full'>
      ...
      <div className='tw-hidden tw-rounded-l-xl tw-relative md:tw-block tw-w-[600px] tw-h-[760px] tw-mx-auto leftsideform'>
            <p className='tw-text-4xl tw-w-full tw-text-white tw-mt-10 tw-absolute tw-text-center  tw-text-6xl'>Welcome to </p> 
            <img src={logo} className='logo-register' alt='szndChef logo'/>
            <Lottie onComplete={() => {
        ...
        <h2 className='tw-text-center tw-text-4xl tw-text-white tw-font-bold tw-pt-[12.5rem] tw-pb-[2.5rem]  tw-text-[3rem]'>Register</h2>
          <div className='tw-flex tw-flex-col tw-py-1.5 tw-text-lg'>
          ...
          <button type='submit' className='tw-border tw-rounded-2xl tw-flex tw-mx-auto tw-justify-center tw-transition tw-ease-in-out tw-text-lg tw-w-1/3 tw-my-2 tw-py-2 tw-text-white tw-font-bold formbutton'>Sign-Up</button>
          <p className='tw-text-center  tw-pt-1 tw-text-lg'>Already have an account? <Link to="/login" className='tw-font-bold tw-transition tw-ease-in-out formlink'>Login</Link> here !</p>
        </form>
      </div>
    </div>
  )
